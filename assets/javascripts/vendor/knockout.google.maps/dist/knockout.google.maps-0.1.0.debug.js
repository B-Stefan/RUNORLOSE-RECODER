define (['knockout','plugins/router', 'jquery','markerclusterer','infobubble'],
    function (ko,router, jQuery,MarkerCluster,Infobubble,unused){
    ...
    ko.google.maps.utils = {
        ...
        bindMapItem: function (bindingContext, element, item) {
            ...
            if(element._cloneTemplate == undefined){
                element._cloneTemplate = jQuery(element).html()
            }

            clone = jQuery("<div></div>").append(jQuery.parseHTML(element._cloneTemplate));
            clone.css("height", "100%")
            clone.css("width", "100%")
            element.appendChild(clone[0])
            ko.applyBindingsToDescendants(childBindingContext, clone[0]);
    ...
    ko.bindingHandlers.map = {
        ...
        binders: {
                ...
                onCreated: function (bindingContext, bindings, map, ko) {
                    if (ko.isObservable(bindings.styles)) {
                        bindings.styles.subscribe(function () {
                            map.setOptions({'styles':bindings.styles()});
                        });
                    }
                }
            },
            resize: {
                onBuildOptions: function (bindingContext, bindings, options, ko) {
                    ko.google.maps.utils.assignBindingToOptions(bindings, 'resize', options, null);
                },
                onCreated: function (bindingContext, bindings, map, ko) {

                    if (ko.isObservable(bindings.resize)) {
                        bindings.resize.subscribe(function () {
                            if (ko.isObservable(bindings.center)){
                                oldCenter = bindings.center()
                            }
                            else {
                                oldCenter = bindings.center
                            }
                            ...
                            if (ko.isObservable(bindings.center)){
                                bindings.center.valueHasMutated(bindings.center())
                            }
                            map.setCenter(ko.google.maps.utils.positionToGoogleMaps(oldCenter))
    ...
    ko.bindingHandlers.infoWindow = {
        init: function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
            ...
            options.borderRadius = (bindings.borderRadius != undefined) ? bindings.borderRadius : undefined;
            options.borderWidth = (bindings.borderWidth != undefined) ? bindings.borderWidth : undefined;
            options.padding  = (bindings.padding != undefined)      ? bindings.padding : undefined;
            options.maxWidth = (bindings.maxWidth != undefined)       ? bindings.maxWidth : undefined;
            options.minWidth = (bindings.minWidth != undefined)       ? bindings.minWidth : undefined;
            options.maxHeight = (bindings.maxHeight != undefined)     ? bindings.maxHeight : undefined;
            options.minHeight = (bindings.minHeight != undefined)    ? bindings.minHeight : undefined;
            options.showCloseButton = (bindings.showCloseButton != undefined)    ? bindings.showCloseButton : undefined;

            infoWindow = new Infobubble(options)
